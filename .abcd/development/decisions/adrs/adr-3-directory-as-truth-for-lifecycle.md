...
- The issue ledger (fn-20, itd-4) applies the same pattern: `.abcd/development/activity/issues/{open,resolved,wontfix}/iss-N-<slug>.md` carries no `status:` field; the sub-directory IS the lifecycle state. A `wontfix_reason:` or `resolution:` field is required in the corresponding directory (enforced by `_issue_lib._validate_invariants`, not by schema location alone), keeping per-state context legible.
