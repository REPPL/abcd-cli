...
# ADR-12: `.work/issues.md` stays the live operational ledger; the structured per-issue store is deferred until the native spec layer schedules the migration (historical)

## Context

abcd records issues discovered during any work in `.work/issues.md` (historical) — the
operational working ledger, append-as-you-find, per the workspace's
issue-recording discipline. `.work/` (historical) is git-ignored, so this ledger is a
...
1. **A structured per-issue store is designed.** A migration spec plans the move
   of the flat `.work/issues.md` (historical) ledger into structured per-issue files under
   `.abcd/development/activity/issues/{open,resolved,wontfix}/iss-*.md` (historical), with
...
2. **AC evidence routed only to `.work/` (historical) is invisible to PR reviewers** (the
   gitignore makes the ledger unreviewable from a diff). That tension is what
   makes "is `.work/issues.md` (historical) the durable record, or a staging surface?" a real
...
ADR, *not* `.work/issues.md` (historical) itself (which, being git-ignored, can never be the
checker-assertable artefact).

**Current-state evidence:**

- The structured-ledger root **does not exist**:
  `.abcd/development/activity/issues/` (historical) is absent (no `activity/` tree at all).
...
- The flat ledger **is live and in active use**:
  `.work/issues.md` (historical) carries current-session operational entries.
...
We will keep **`.work/issues.md` (historical) as the live operational issue ledger**, and
...
(`/abcd:capture → iss-N` plus the `.abcd/development/activity/issues/` (historical) store)
happens **when, and only when, a future native spec explicitly schedules the
`.work/issues.md` (historical) → `iss-N` migration** into a phase and runs it. Absent that
...
**not** legitimise `.work/` (historical) as a durable routing target: `.work/issues.md`
...
1. **Run the structured migration now.** Build the structured
   `.abcd/development/activity/issues/` (historical) store, migrate every `.work/issues.md`
...
2. **Record the decision as a free-form `.work/` (historical) note.** Rejected: `.work/` is
   ...
   be committed. A `.work/` (historical) note would re-commit the exact failure mode the
...
- The live issue-capture workflow is unchanged and unblocked — contributors and
  agents keep appending to `.work/issues.md` (historical) with no migration friction.
...
**Costs / obligations:**
- AC evidence and routed follow-ups recorded *only* in `.work/issues.md` (historical) remain
