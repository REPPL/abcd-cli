...
**Deferred to the prompt-test spec — the broader structural checks:** missing role definitions, missing output schemas, vague instructions, missing example I/O, prompt-length outliers, missing `## Research basis: <path>` / `## Last SOTA Audit: <date>` footers. These role/schema/example-IO structural checks are not in fn-8's scope; they ship alongside the B-layer `internal/core/prompttest` golden-test harness. The linter runs pre-commit. It catches structural issues; it does not catch semantic quality.
