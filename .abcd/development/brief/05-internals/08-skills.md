...
- A **per-invocation report subdirectory** (`.abcd/.work.local/logs/<verb>/<timestamp>/`) for commands that emit run reports.
...
abcd ships **eleven binary-backed top-level commands**, each with both a Go verb and a `commands/abcd/` file: `/abcd:ahoy`, `/abcd:audit`, `/abcd:capture`, `/abcd:disembark`, `/abcd:docs`, `/abcd:embark`, `/abcd:history`, `/abcd:intent`, `/abcd:launch`, `/abcd:memory`, and `/abcd:version`. The `intent` parent carries the largest sub-verb tree: `link`, `plan`, `ready`, and `review` ship (alongside the deprecated `new` alias and the canonical bare quoted create `/abcd:intent "<text>"`); `refine`, `grill`, `ship`, `consistency`, `shape`, and `reclassify` are design targets. Three further binary verbs — `changelog`, `rules`, and `spec` — expose a Go verb but no `commands/abcd/` file, so they are CLI-only. See [`04-surfaces/`](../04-surfaces) for per-command detail.
