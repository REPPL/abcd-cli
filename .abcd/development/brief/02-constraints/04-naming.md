...
| `rules.json` | Repo-scope rule overrides file at `<repo>/.abcd/rules.json`. Named in the in-repo carve-out in `05-internals/03-configuration.md` § The two `.abcd/` scopes. JSON Schema 2020-12; schema at `scripts/abcd/schemas/rules.schema.json`. | spc-14 spec + `spc-14-modular-rules-loader-prompt-router-hook.1` |
