...
> **Delivery state**: `/abcd:intent` is a design-target surface — no `intent` verb exists on the shipped binary or plugin command surface. Its backing intents sit in `intents/planned/` (itd-27 grill, itd-34 kinds, itd-46 quoted create, itd-48 reviewer roles 2–3, itd-50 audit loop) and `intents/drafts/` (itd-16, itd-35 — the `/abcd:audit` sub-verbs); delivery state is the intent lifecycle's, not this page's (see the [brief README's provenance note](../README.md)).
