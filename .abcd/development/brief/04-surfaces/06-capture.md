...
- **Given** an existing issue at `.abcd/development/activity/issues/open/iss-3-foo.md`, **when** the user runs `/abcd:capture resolve iss-3 "fixed in spc-7 task 4"`, **then** the file moves to `.abcd/development/activity/issues/resolved/iss-3-foo.md` with the resolution recorded.
...
- **`.work/issues.md` migration:** delivered by `spc-22-workissuesmd-migration-promote-legacy`.
