...
Per [adr-9](../../decisions/adrs/adr-9-phase-as-product-layer.md), the authoritative intent set and its execution order — including per-intent dependencies and the bundling into product phases — live in the phase docs and the intent index, not in this file. See [`roadmap/phases/README.md`](../../roadmap/phases/README.md) for the ordered phase plan (each phase doc names the intents it bundles via its `## Maps against`) and [`roadmap/intents/README.md`](../../roadmap/intents/README.md) for the intent corpus with kinds and lifecycle state. The dependency DAG governs what can run when; abcd's autonomous loop picks ready intents up in dependency order, with parallelism where dependencies allow.
...
Post-brief additions (itd-27, itd-28, itd-34) are not enumerated here; they ship per their own dependencies — see [`roadmap/phases/`](../../roadmap/phases/README.md) for the phase plan and [`intents/README.md`](../../roadmap/intents/README.md) for the intent index.
