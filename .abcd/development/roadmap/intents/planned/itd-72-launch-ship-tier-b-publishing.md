...
> version and changelog into the public snapshot, prunes superseded payload
> files, stages the delta for confirmation, and then pushes branch + tag
> atomically. If nothing changed, it refuses. If the public repo is not the
> repo it expects, it refuses. If the push fails, nothing half-lands: a
> recoverable marker preserves the local commit and `main` is never
> force-reset.

## Why This Matters

Tier A (itd-67) makes the public repo installable and versioned; without
Tier B every subsequent release is a hand-run of renders, bumps, changelog
edits, prunes, and pushes — each a chance to publish a half-state. `ship`
makes the release cut a single audited act with every safety refusal
built in.

## What's In Scope

- Bump-tier computation from the public `.launch/launch-report.json` baseline
...
- No-change refusal on the canonical pre-bump payload digest (`--force` to
  override)
- Public-repo identity check (marketplace name, plugin name, remote slug,
...
- **Given** no baseline exists in the public repo, **when** `ship` runs,
...
- **Given** the pre-bump payload digest equals the baseline's, **when** `ship`
  ...
  publishes nothing.
- **Given** the public repo's marketplace name, plugin name, remote slug, or
