...
2. **Autonomous loops** (Ralph and successors, contextualised by itd-29) — long-running agents that the user kicks off and walks away from. They share a working tree with whatever the user does next.
...
- **Typed `work_item` claims.** The unit of coordination is a typed work item, not a file region:
  ...
  - `spec_task(fn-N.M)` — exclusive at task granularity, **not spec granularity**. Agents A and B can both work `fn-7` if on different tasks (`.2` and `.3`); they collide only if they pick the same task.
...
- **Given** agent A holds `spec_task(fn-7.2)`, **when** agent B calls `take(spec_task(fn-7.3))`, **then** the claim succeeds (task-level granularity, not spec-level) — both agents progress different tasks of the same spec concurrently.
...
- Methodological precedents: file locks (`flock`/`fcntl`); the actor model (Erlang/OTP); the Ralph autonomous loop pattern (single-agent today, multi-agent scaling tomorrow). The earlier draft's reference to LangGraph / CrewAI / AutoGen as substrate candidates has been dropped — the narrowed scope needs none of them.
