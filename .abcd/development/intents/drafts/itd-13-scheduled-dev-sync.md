...
> **abcd keeps `.abcd/development/activity/` in sync with its volatile sources continuously.** A scheduled launchd agent (or cron entry) runs `abcd dev-sync` every hour by default, automatically promoting new RP chats into `.abcd/development/activity/reviews/`, distilling new Claude memories into `.abcd/memory/`, and curating `.work/issues.md` entries into `.abcd/development/activity/issues/`. By the time you run `/abcd:disembark`, everything is already current.
>
> "Sync ran only at disembark, which meant my lifeboats were always missing the last few hours of oracle reviews," said Frank, SRE. "Now it syncs in the background. When I disembark, I know what's in `.abcd/development/activity/` is what's in my recent work."
...
Once real usage patterns emerge, the cost of "stale `.abcd/development/activity/` until next disembark" will become visible. This intent closes that gap with a real scheduled-sync solution.
...
- Per-source enable/disable (`dev_sync.{rp,memory,work}.enabled`)
...
- Cross-machine sync (RP chats from one machine appearing in another's `.abcd/development/activity/`)
- Pull-based sync from external sources beyond the current set (RP, memory, .work)
...
- **Given** a scheduled `dev-sync` run conflicts with an in-progress `/abcd:disembark to <path>` (file-system contention on `.abcd/development/activity/`), **when** the scheduler fires, **then** the scheduled run detects the active disembark via a documented lock mechanism, skips its work, logs the skip-reason to its run report, and waits for the next interval.
