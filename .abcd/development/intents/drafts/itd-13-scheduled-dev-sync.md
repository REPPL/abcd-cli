...
> **abcd keeps `.abcd/development/activity/` in sync with its volatile sources continuously.** A scheduled launchd agent (or cron entry) runs `abcd dev-sync` every hour by default, promoting new chats from whichever oracle adapter is configured into `.abcd/development/activity/reviews/`, distilling new Claude memories into `.abcd/memory/`, and curating `.work/issues.md` entries into `.abcd/development/activity/issues/`. Each source is adapter-scoped: dev-sync pulls only from the sources that are actually configured. By the time you run `/abcd:disembark`, everything is already current.
>
...
- Cross-machine sync (oracle-adapter chats from one machine appearing in another's `.abcd/development/activity/`)
