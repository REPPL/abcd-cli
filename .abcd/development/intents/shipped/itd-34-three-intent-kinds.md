...
- **Bundle-member**: multi-arg `/abcd:intent plan itd-A itd-B`. `/flow-next:plan` once with all intents as joint input; `epic.intent: [itd-A, itd-B]`; each intent's `epic_id: fn-N` and `bundle: <bundle-id>`; all members `drafts/` → `planned/` together.
