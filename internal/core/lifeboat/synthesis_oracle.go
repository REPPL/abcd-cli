...
// ---------------------------------------------------------------------------
// INTEGRATION NOTE (M6 build-order): the five helpers below are the SHARED
// synthesis helpers the design (§9) assigns to Agent A1's synthesis_principles.go,
// reused verbatim by A2. In this isolated worktree A1's file is not present, so
// they are provided here — behind the agreed signatures — so the oracle seam
// compiles and its tests run. At integration KEEP EXACTLY ONE copy: delete this
// block once A1's synthesis_principles.go lands (both are the same logic, each a
// faithful mirror of the existing graveyard_lessons.go / embark.go patterns).
// ---------------------------------------------------------------------------

// gateSynthLifeboat gates a post-pack lifeboat for a synthesis verb: a real
// directory carrying a parseable _provenance.json, no newer than this abcd
// understands. It returns the absolute path and the immutable provenance header.
...
// buildLifeboatPathSet walks the packed lifeboat (bounded, symlink-refusing, sorted)
// and returns the set of every packed relative POSIX path — the live path set an
// evidence ref is validated against.
...
// cleanSynthProse sanitises untrusted synthesis prose: it collapses newlines and
// neutralises HTML-comment delimiters (so the prose can neither break its line nor
// forge an abcd marker), strips control characters, then caps the length. An empty
// result signals the entry should be dropped. Mirrors cleanLessonProse.
func cleanSynthProse(s string) string {
	...
	if len(s) > maxSynthProseBytes {
		s = strings.ToValidUTF8(s[:maxSynthProseBytes], "")
		s = strings.TrimSpace(s)
	}
	return s
}

// filterSynthEvidence keeps only the refs that resolve in valid, deduped and
// order-preserving, reading at most maxSynthEvidenceRefs entries. Mirrors
// filterEvidence.
...
// newline), matching the plan.go / marshalLessonsFile convention.
